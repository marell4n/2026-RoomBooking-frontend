window.RoomAdmin = {
    openModal(id = null) {
        const room = id ? window.allRooms.find(r => r.id === id) : null;
        const modalHTML = `
                ...
                <div class="bg-white w-full max-w-md p-6 rounded-2xl shadow-xl">
                    ...
                    <form id="form-room" class="mb-4 mt-4">
                        ...
                        <div class="flex gap-2">
                            <button type="button" onclick="RoomAdmin.closeModal()" class="flex-grow border py-2 rounded-lg">Batal</button>
    ...
    async delete(id) {
        ...
        if (res.ok) RoomUser.renderList('main-content');
    }
};